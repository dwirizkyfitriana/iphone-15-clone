...
const VideoCarousel = () => {
  ...
  const videSpanRef = useRef<HTMLSpanElement[]>([])
      ...
      let anim = gsap.to(span[videoId], {
        onUpdate: () => {
          ...
          if (progress !== currentProgress) {
            ...
            gsap.to(videoDivRef.current[videoId], {
              width: window.innerWidth < 760 ? '10vw' : window.innerWidth < 1200 ? '10vw' : '2vw',
            })

            gsap.to(videSpanRef.current[videoId], {
        ...
        onComplete: () => {
          if (isPlaying) {
            ...
            gsap.to(videSpanRef.current[videoId], { backgroundColor: '#afafaf' })
          }
        },
      })
  ...
  const handleProcess = (
    type: 'video-end' | 'video-reset' | 'video-last' | 'play' | 'pause',
    i?: number
  ) => {
    switch (type) {
      case 'video-end':
        setVideo((prevVideo) => ({ ...prevVideo, isEnd: true, videoId: (i || 0) + 1 }))
        break
      case 'video-last':
          ...
          {videoRef.current.map((_, i) => (
            <span
              ...
              <span
                ...
                ref={(el: HTMLSpanElement) => (videSpanRef.current[i] = el)}></span>
            </span>
          ))}
        </div>
